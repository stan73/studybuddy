-- Migration 013: Prüfungsvorbereitung — Karten einem bevorstehenden Prüfungstermin zuordnen
-- Eine „Prüfung" = Karten mit gleichem exam_name + exam_date. Treibt Lernplan + Übersicht.
-- RPCs übertragen exam_name/exam_date im cards-JSONB (keine Signaturänderung).
-- ─────────────────────────────────────────────────────────────────────────

ALTER TABLE cards ADD COLUMN IF NOT EXISTS exam_name TEXT CHECK (exam_name IS NULL OR char_length(exam_name) <= 100);
ALTER TABLE cards ADD COLUMN IF NOT EXISTS exam_date DATE;
CREATE INDEX IF NOT EXISTS cards_exam ON cards(user_id, exam_date) WHERE exam_date IS NOT NULL;
...
BEGIN
  ...
  INSERT INTO cards (id, user_id, subject, front, back, level, due_at, ease, reps, interval_days, topic, exam_name, exam_date)
    SELECT COALESCE(NULLIF(c->>'id','')::uuid, gen_random_uuid()),
           ...
           NULLIF(c->>'exam_name',''), NULLIF(c->>'exam_date','')::date
...
BEGIN
  ...
  RETURN json_build_object(
    'cards', COALESCE((SELECT json_agg(row_to_json(x)) FROM
      (SELECT id, subject, front, back, level, due_at, ease, reps, interval_days, topic, exam_name, exam_date FROM cards WHERE user_id = v_uid AND child_id IS NULL) x), '[]'::json),
...
BEGIN
  ...
  INSERT INTO cards (id, user_id, child_id, subject, front, back, level, due_at, ease, reps, interval_days, topic, exam_name, exam_date)
    SELECT COALESCE(NULLIF(c->>'id','')::uuid, gen_random_uuid()),
...
BEGIN
  ...
  RETURN json_build_object(
    'cards', COALESCE((SELECT json_agg(row_to_json(x)) FROM
      (SELECT id, subject, front, back, level, due_at, ease, reps, interval_days, topic, exam_name, exam_date FROM cards WHERE child_id = p_child_id) x), '[]'::json),
    'tasks', COALESCE((SELECT json_agg(row_to_json(x)) FROM
