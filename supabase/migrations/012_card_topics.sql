-- Migration 012: Lernthemen pro Karteikarte (topic) innerhalb eines Fachs
-- Karten lassen sich Themengebieten zuordnen und gezielt nach Thema üben.
-- topic NULL/'' = „Allgemein". RPCs übertragen topic im cards-JSONB (keine Signaturänderung).
-- ─────────────────────────────────────────────────────────────────────────

ALTER TABLE cards ADD COLUMN IF NOT EXISTS topic TEXT CHECK (topic IS NULL OR char_length(topic) <= 80);
CREATE INDEX IF NOT EXISTS cards_user_subject_topic ON cards(user_id, subject, topic);
...
BEGIN
  ...
  INSERT INTO cards (id, user_id, subject, front, back, level, due_at, ease, reps, interval_days, topic)
...
BEGIN
  ...
  RETURN json_build_object(
    'cards', COALESCE((SELECT json_agg(row_to_json(x)) FROM
      (SELECT id, subject, front, back, level, due_at, ease, reps, interval_days, topic FROM cards WHERE user_id = v_uid AND child_id IS NULL) x), '[]'::json),
...
BEGIN
  ...
  INSERT INTO cards (id, user_id, child_id, subject, front, back, level, due_at, ease, reps, interval_days, topic)
    SELECT COALESCE(NULLIF(c->>'id','')::uuid, gen_random_uuid()),
...
BEGIN
  ...
  RETURN json_build_object(
    'cards', COALESCE((SELECT json_agg(row_to_json(x)) FROM
      (SELECT id, subject, front, back, level, due_at, ease, reps, interval_days, topic FROM cards WHERE child_id = p_child_id) x), '[]'::json),
    'tasks', COALESCE((SELECT json_agg(row_to_json(x)) FROM
